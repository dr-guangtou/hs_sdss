...
pro hs_ppxf_emi_subtract, name_str, imf_index, flux_sub, flux_res, $
    ...
    file1 = name_str + '_' + imf_index + '_1_ppxf.fits'
    file2 = name_str + '_' + imf_index + '_2_ppxf.fits'
    file3 = name_str + '_' + imf_index + '_3_ppxf.fits'
    ...
    output = name_str + '_' + imf_index + '_emisub.txt' 
...
pro hs_ppxf_emi_fitting, spec_file, temp_list, $
    ...
    spec_txt=spec_txt, location=location, $ 
...
spec = strcompress( spec_file, /remove_all )
;; 
temp = strsplit( spec, '.', /extract ) 
name_str = temp[0]
...
;;
if keyword_set( location ) then begin 
    loc = strcompress( location, /remove_all ) 
    spec = loc + spec 
endif 
if NOT file_test( spec ) then begin 
    print, 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
    print, ' Can not find the input spectrum file : ' + spec + ' !!!' 
    print, 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
    message, ' '
endif
;;
if keyword_set( spec_txt ) then begin 
    if keyword_set( error_arr ) then begin 
        readcol, spec, wave_ori, flux_ori, error_ori, format='F,D,D', $
            delimiter=' ', comment='#', /silent 
    endif else begin 
        readcol, spec, wave_ori, flux_ori, format='F,D', $
            delimiter=' ', comment='#', /silent 
    endelse
endif else begin 
...
;;
hs_setup_miuscat_library, temp_list, velscale, fwhm_data, fwhm_libr, $
...
if keyword_set( save_result ) then begin 
    ...
    result_file = name_str + '_ppxf.fits' 
    print, '###############################################################'
    ...
    ;; The bestfit should be identical with best_temp
    ;; best_diff = ( bestfit - best_temp ) 
...
if keyword_set( debug ) then begin 
    ...
    ;;
    result_plot = name_str + '_ppxf_debug.eps' 
    ;; 
    mydevice = !d.name 
    !p.font=1
    set_plot, 'ps' 
    ...
    ;; 
    cgOplot, wave_lin, bestfit, color=cgColor( 'RED6' ), linestyle=0, thick=1.5
    ;;
    cgOplot, wave_lin, mpoly,   color=cgColor( 'GRN5' ), linestyle=2, thick=1.5
    ;;
...
if keyword_set( lib_location ) then begin 
    ...
    lib_location = '/home/hs/work/data/ssp/miuscat/fits/fwhm251/'
endelse
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
fwhm_data = float( fwhm_data )
fwhm_libr = float( fwhm_libr )
...
pro list_ppxf_emi_remove, list 
    ...
    for ii = 0, ( n_spec - 1 ), 1 do begin 
        ...
        hs_ppxf_emi_remove, spec_file, location=location
