pro hs_plot_sl_output, sl_file, $
    base_vdisp=base_vdisp, feature_over=feature_over, sig_over=sig_over, $
...
if NOT file_test( sl_file ) then begin 
    ...
    if file_test( res_file ) then begin 
        ...
        hs_parse_sl_output, sl_file, sl_struc, base_struc, spec_struc, /quiet, $
            /save_fits, /save_txt 
    endelse 
endelse
;; name of the plot 
...
if keyword_set( sig_over ) then begin 
    for j = 0, ( n_ssp_sig - 1 ), 1 do begin 
        if ( base_struc_sort[j].ssp_find EQ 1 ) then begin 
            ...
            print, '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
            readcol, s_sig, w_sig, f_sig, FORMAT='F,D', /silent, comment='#', $ 
                delimiter=' ' 
            ...
            log_rebin, lam_range, f_sig_inter, f_sig_log, loglam, /oversample, $
                velscale=velscale 
...
x_step = 0.04 
label = 'Observed'
cgPlots, [ x_pos, (x_pos+x_step) ], [ y_pos, y_pos ], linestyle=0, thick=10.0, $
    color=cgColor( 'Gray' ), /normal
...
x_step = 0.04 
label = 'Synthetic'
cgPlots, [ x_pos, (x_pos+x_step) ], [ y_pos, y_pos ], linestyle=0, thick=13.0, $
...
if keyword_set( zoomin ) then begin 
    ...
    y3 = ( ( max( spec_obs[ where( ( wave GE ( wrange1[0] - 3 ) ) AND $
        ...
        ( flux_range[1] - flux_range[0] ) * ( position_1[3] - position_1[1] ) + $
        position_1[1]
    ...
    cgPlot, wave, spec_obs, xstyle=1, ystyle=1, charthick=8.0, charsize=2.0, $
        xthick=12.0, ythick=12.0, /noerase, position=position_3, /nodata, $ 
        xrange=wrange1, yrange=frange1, xticklen=0.05, yticklen=0.02, $
        yminor=-1, xtickformat='(A1)'
    ...
    cgAxis, xaxis=1.0, xrange=wrange1, xstyle=1, charsize=2.5, xthick=10.0, $
        xticklen=0.05
    cgPlot, wave, spec_obs, linestyle=0, thick=2.0, color=cgColor('Dark Gray'), $ 
        /overplot 
    ...
    if keyword_set( feature_over ) then begin 
        for j = 0, ( n_features - 1 ), 1 do begin 
            ...
            if ( ( f_lam0 GT wrange1[0] ) AND ( f_lam1 LE wrange1[1] ) ) $
                ...
                if ( ( l_pos_x0 - l_pos_tmp ) GE $
                    ( ( wrange1[1] - wrange1[0] ) / 12.0 ) ) then begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 8.0 ) ) 
                endif else begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 5.0 ) ) 
                endelse
                l_pos_tmp = l_pos_x0
                cgText, l_pos_x0, l_pos_y0, f_name, charsize=2.0, charthick=9.0, $
                    alignment=0.5, color=cgColor( 'Black' ), /data 
            endif
        endfor
    endif
    cgPlot, wave, spec_obs, xstyle=1, ystyle=1, charthick=8.0, charsize=2.0, $
    ...
    cgPlot, wave, spec_obs, xstyle=1, ystyle=1, charthick=8.0, charsize=2.0, $
        xthick=12.0, ythick=12.0, /noerase, position=position_4, /nodata, $ 
        xrange=wrange2, yrange=frange2, xticklen=0.05, yticklen=0.02, $
        yminor=-1, xtickformat='(A1)'
    ;; First put the highlighted features on the background: 
    if keyword_set( feature_over ) then begin 
        for j = 0, ( n_features - 1 ), 1 do begin 
            ...
            y0 = !Y.Crange[0] 
            y1 = !Y.Crange[1]
            if ( ( f_lam0 GT wrange2[0] ) AND ( f_lam1 LE wrange2[1] ) ) $
                then begin 
                if ( features[j].ind EQ 1 ) then begin 
                    polygon_x = [ f_lam0, f_lam1, f_lam1, f_lam0, f_lam0 ]
                    polygon_y = [ y0, y0, y1, y1, y0 ]
    ...
    cgPlot, wave, spec_syn, linestyle=0, thick=6.0, color=cgColor('Red'), $ 
        /overplot 
    cgAxis, xaxis=0.0, xrange=wrange2, xstyle=1, charsize=2.5, xthick=10.0, $
        xtickformat='(A1)', xticklen=0.05
    cgAxis, xaxis=1.0, xrange=wrange2, xstyle=1, charsize=2.5, xthick=10.0, $
        xticklen=0.05
    if keyword_set( feature_over ) then begin 
        for j = 0, ( n_features - 1 ), 1 do begin 
            ;; decide the color of the lines around the features 
            ...
            if ( ( f_lam0 GT wrange2[0] ) AND ( f_lam1 LE wrange2[1] ) ) $
                ...
                l_pos_x0 = ( ( f_lam0 + f_lam1 ) / 2.0 ) 
                if ( ( l_pos_x0 - l_pos_tmp ) GE $
                    ( ( wrange2[1] - wrange2[0] ) / 12.0 ) ) then begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 8.0 ) ) 
                endif else begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 5.0 ) ) 
                endelse
    ...
    cgPlot, wave, spec_obs, xstyle=1, ystyle=1, charthick=8.0, charsize=2.0, $
        xthick=12.0, ythick=12.0, /noerase, position=position_5, /nodata, $ 
        xrange=wrange3, yrange=frange3, xticklen=0.05, yticklen=0.02, $
        yminor=-1, xtickformat='(A1)'
    ;; First put the highlighted features on the background: 
    if keyword_set( feature_over ) then begin 
        for j = 0, ( n_features - 1 ), 1 do begin 
            ...
            y0 = !Y.Crange[0] 
            y1 = !Y.Crange[1]
            if ( ( f_lam0 GT wrange3[0] ) AND ( f_lam1 LE wrange3[1] ) ) $
                then begin 
                if ( features[j].ind EQ 1 ) then begin 
                    polygon_x = [ f_lam0, f_lam1, f_lam1, f_lam0, f_lam0 ]
                    polygon_y = [ y0, y0, y1, y1, y0 ]
    ...
    cgPlot, wave, spec_syn, linestyle=0, thick=6.0, color=cgColor('Red'), $ 
        /overplot 
    cgAxis, xaxis=0.0, xrange=wrange3, xstyle=1, charsize=2.5, xthick=10.0, $
        xtickformat='(A1)', xticklen=0.05
    cgAxis, xaxis=1.0, xrange=wrange3, xstyle=1, charsize=2.5, xthick=10.0, $
        xticklen=0.05
    if keyword_set( feature_over ) then begin 
        for j = 0, ( n_features - 1 ), 1 do begin 
            ;; decide the color of the lines around the features 
            ...
            if ( ( f_lam0 GT wrange3[0] ) AND ( f_lam1 LE wrange3[1] ) ) $
                ...
                l_pos_x0 = ( ( f_lam0 + f_lam1 ) / 2.0 ) 
                if ( ( l_pos_x0 - l_pos_tmp ) GE $
                    ( ( wrange3[1] - wrange3[0] ) / 12.0 ) ) then begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 8.0 ) ) 
                endif else begin 
                    l_pos_y0 = ( y1 - ( ( y1 - y0 ) / 5.0 ) ) 
                endelse
...
cgPlots, polygon_x, polygon_y, linestyle=0, thick=12.0, color=cgColor('Black'), $ 
    /normal
left  = position_6[0]
right = position_6[2]
lower = position_6[1]
...
label = 'INPUT: ' + sl_struc.spec_name 
cgText, xloc, yloc, label, charsize=2.8, charthick=10.0, color=cgColor('Black'),$
    alignment=0.5, /normal 
;; Output spectrum 
xloc = xmiddle
yloc = upper - ( ystep * 1.8 ) 
label = 'OUTPUT: ' + sl_struc.output_name 
cgText, xloc, yloc, label, charsize=3.0, charthick=10.0, color=cgColor('Black'),$
    alignment=0.5, /normal 
;; Base name  
xloc = xmiddle
...
cgText, xloc, yloc, label, charsize=3.0, charthick=9.0, color=cgColor('Black'),$
    alignment=0.0, /normal 
;; Extinction name  
xloc = xmiddle - ( xstep * 0.5 ) 
yloc = upper - ( ystep * 5.0 ) 
...
label = 'Chi2/N: ' + $
    strcompress( string( sl_struc.reduced_chi2, format='(F9.3)' ), /remove_all ) 
cgText, xloc, yloc, label, charsize=3.0, charthick=9.0, color=cgColor('Black'),$
    alignment=0.0, /normal 
;; Adev  
xloc = xmiddle - ( xstep * 0.5 )
...
cgText, xloc, yloc, label, charsize=2.6, charthick=9.0, color=cgColor('Black'),$
    alignment=0.0, /normal 
;; BIC  
xloc = xmiddle - ( xstep * 0.5 )
yloc = upper - ( ystep * 7.0 ) 
...
xloc = xmiddle - ( xstep * 0.5 )
yloc = upper - ( ystep * 8.0 ) 
label = 'age_flux:' + string( ( sl_struc.at_flux / 1.0D9 ), format='(F8.2)' ) 
cgText, xloc, yloc, label, charsize=3.0, charthick=9.0, color=cgColor('Black'),$
    alignment=0.0, /normal 
;; at_mass   
xloc = xmiddle - ( xstep * 0.5 )
yloc = upper - ( ystep * 9.0 ) 
label = 'age_mass:' + string( ( sl_struc.at_mass / 1.0D9 ), format='(F7.2)' ) 
cgText, xloc, yloc, label, charsize=3.0, charthick=9.0, color=cgColor('Black'),$
    alignment=0.0, /normal 
;; am_flux    
xloc = xmiddle - ( xstep * 0.5 )
...
frac_inter = ceil( ceil( ( frac_range[1] - frac_range[0] ) / 0.1 ) / 4.0 ) * 0.1
...
cgPlot, age_ind_arr, flux_norm_age, xstyle=1, ystyle=1, position=position_9, $
    xrange=age_range, yrange=frac_range, xthick=12.0, ythick=12.0, $
    /nodata, /noerase, xtickformat='(A1)', $ 
    xticklen=0.03, yticklen=0.02, xminor=-1, yminor=-1, $ 
...
cgPlot, age_ind_arr, mcor_norm_age, xstyle=1, ystyle=1, position=position_10, $
    xrange=age_range, yrange=frac_range, xthick=12.0, ythick=12.0, $
    /nodata, /noerase, ytickformat='(A1)', xtickformat='(A1)', $ 
    xticklen=0.03, yticklen=0.02, xminor=-1, yminor=-1, $ 
...
if keyword_set( topng ) then begin 
    ...
    plot_png = out_string + '_result.png' 
