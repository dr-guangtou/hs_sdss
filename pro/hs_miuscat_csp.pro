function hs_miuscat_csp, miuscat_file,  $ ;; The file including the miuscat SSPs 
    imf=imf, met=met, $               ;; The IMF and metallicity for CSP 
    ts=ts, np=np, tau=tau, tr=tr, $       ;; SFH parameters 
    t_cosmos=t_cosmos, n_time=n_time, $   ;; Hubble time and number of time frames 
    debug=debug, savefits=savefits  
    ...
    ;; Define the float array for unqi metallicity 
    met_uniq  = [ -2.32, -1.71, -1.31, -0.71, -0.40, 0.00, 0.22 ]
    ;; Also the string labels for metallicity for the output file
    met_label = [ 'z1', 'z2', 'z3', 'z4', 'z5', 'z6', 'z7' ]
    n_met_uniq = 7
    ...
    ;; 1. IMF type
    if NOT keyword_set( imf ) then begin 
    ...
    ;; 2. Metallicity value
    if NOT keyword_set( met ) then begin 
        ...
        n_met = n_elements( met ) 
        met = met[ uniq( met, sort( met ) ) ]
        for i = 0, ( n_met - 1 ), 1 do begin 
            if ( where( met_uniq EQ met[i] ) EQ -1 ) then begin 
                print, 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'
    ...
    for ii = 0, ( n_imf - 1 ), 1 do begin 
        ...
        for jj = 0, ( n_met - 1 ), 1 do begin 
            met_now = met[ jj ]
            met_str = met_label[ where( met_uniq EQ met_now ) ]
            ;; 3. SFH level
            for kk = 0, ( n_ts - 1 ), 1 do begin 
                ...
                if ( do_truncation EQ 0 ) then begin 
                    sfh_struc = hs_sfh_generate( ts[kk], np[kk], tau[kk], $
                        n_time=n_time, t_cosmos=t_cosmos, /plot ) 
                endif else begin 
                    sfh_struc = hs_sfh_generate( ts[kk], np[kk], tau[kk], $
                        t_trunc=tr[kk], $
                        n_time=n_time, t_cosmos=t_cosmos, /plot ) 
                ...
                ;; XXX
                ;time_arr = sfh_struc.time
                ;sfr_arr  = sfh_struc.sfr
                ;print, 'XXX SFH Check XXX'
                ;for nn = 0, ( n_time - 1 ), 1 do begin 
                ;    print, (nn+1), time_arr[nn], sfr_arr[nn]
                ;endfor
                ...
                ;; Test XXXX 
                ;csp_temp = im_convolve_sfh_mod( ssp_struc, $
                ;    sfh=sfr_arr, time=time_arr, mstar=mstar_arr, $
                ;    cspmstar=cspmstar, age_mw=age_mw, age_lw=age_lw ) 
                ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
                ;; Time check 2 
                if keyword_set( debug ) then begin 
                    print, '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
