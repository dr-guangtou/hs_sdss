; + 
; NAME:
;              HS_STARLIGHT_READ_OUT
;
; PURPOSE:
;              Parse the output file of Starlight.V04 into IDL structures 
;
; USAGE:
;    hs_starlight_read_out, out_file, sl_struc, base_struc, spec_struc, $ 
...
;             Song Huang, 2014/06/15 - Minor modification; Remove AIC/BIC 
;-
...
function hs_retrieve_para, chunk, tag, sep, num
    ...
    if ( n_elements( line ) EQ 1 ) then begin 
        ...
        print, '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
        print, '  Be careful, there are more than one line that includes: ' $
            + tag 
        print, '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
        para = !VALUES.F_NaN
    endelse
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro hs_starlight_read_out, sl_output, sl_struc, base_struc, spec_struc, $
    save_fits=save_fits, save_txt=save_txt, quiet=quiet, base_dir=base_dir, $ 
    is_fxk=is_fxk, hvdisp_home=hvdisp_home
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Exam and read in the output sl_output 
    sl_output = strcompress( sl_output, /remove_all ) 
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if NOT file_test( sl_output ) then begin 
        print, '############################################################'
        print, '  Can not find ' + sl_output + ' ! Please Check ! '
        print, '############################################################'
        message, ' '
    endif else begin
        ;; Pull out the string that identifies this output sl_output 
        out_string = strsplit( sl_output, ' .', /extract )
        out_string = strcompress( out_string[0], /remove_all )
        ;; Read in the output sl_output into a string array 
        num_lines = file_lines( sl_output ) 
        sl_outarr = strarr( num_lines ) 
        openr, lun, sl_output, /get_lun
        readf, lun, sl_outarr 
        close, lun 
        free_lun, lun
        ...
        check_posit = strpos( sl_outarr, check_array )
    ...
    sl_struc = { output_name:'', spec_name:'', base_name:'', mask_name:'', $
        ...
        reduced_chi2:0.0D, total_chi2:0.0D, adev1:0.0D, adev2:0.0D, $
        avg_delta:0.0D, rms_res:0.0D, $
        xj_sum:0.0D, flux_tot:0.0D, $
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    sl_struc.output_name = out_file 
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    
    if NOT keyword_set( quiet ) then begin 
        print, '###############################################################'
        print, '  Read in STARLIGHT v4 output file: ' + out_file + ' !!!' 
        print, '###############################################################'
    endif
    
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Separate the first chunk, which is the summary of the input,           ;;
    ;;   configuration settings, and the results                              ;;
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    posit_0 = strpos( sl_outarr, check_0 ) 
    ...
    posit_1 = strpos( sl_outarr, check_1 ) 
    ...
    sum_chunk = sl_outarr[ sum_chunk_0 : sum_chunk_1 ]
    ...
    tag = '[Clip-Bug, RC-Crash & Burn-In warning-flags, '
    sl_struc.n_burnin_warning = long( hs_retrieve_para( sum_chunk, tag, ' ', $
        2 ) )
    if NOT keyword_set( QUIET ) then begin 
    ...
    tag = '[Clip-Bug, RC-Crash & Burn-In warning-flags, '
    sl_struc.wei_nsig_threshold = float( hs_retrieve_para( sum_chunk, tag, $
        ' ', 4 ) )
    if NOT keyword_set( QUIET ) then begin 
    ...
    tag = '[adev (%)]'
    sl_struc.adev1 = float( hs_retrieve_para( sum_chunk, tag, ' ', 0 ) )
    if NOT keyword_set( QUIET ) then begin 
        print, ' ##    ADEV1    :  ' + strcompress( $
            string( sl_struc.adev1 ), /remove_all )
    endif
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;;   c) Sum. of x 
    tag = '[sum-of-x (%)]'
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Get the total chi^2
    sl_struc.total_chi2 = ( sl_struc.reduced_chi2 * sl_struc.nl_eff )
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; AIC / BIC only applies to linear models....
    ...
    ;; Extract the summary chunk 
    base_chunk = sl_outarr[ base_chunk_0 : base_chunk_1 ]
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    for i = 0, ( nlobs - 1 ), 1 do begin 
        ...
        spec_struc[i].spec_res = ( spec_struc[i].spec_obs - $
            spec_struc[i].spec_syn ) 
        weight = long( temp[3] ) 
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    index_use = where( ( spec_struc.final_mask NE 1 ) AND $
                       ( spec_struc.pixel_clip NE 1 ) )
    n_pix_use = n_elements( index_use ) 
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Get the RMS of residual 
    resistant_mean, spec_struc[ index_use ].spec_res, 3.5, avg_res, sig_res, $
        num_rej 
    sl_struc.rms_res = sig_res
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; Get the modified ADEV 
        ...
        for i = 0, ( n_base - 1 ), 1 do begin 
            ...
            if ( base_struc[i].ssp_use EQ 1 ) then begin 
                if ( use_base EQ 1 ) then begin 
                    base_struc[i].ssp_loc = base_dir + '/' + $
                        base_struc[i].ssp_file 
                ...
                if ( base_struc[i].ssp_find EQ 0 ) then begin 
                    ...
                    if ( n_find GT 1 ) then begin 
                        for k = 0, ( n_find - 1 ), 1 do begin 
                            file = strlowcase( strcompress( temp[k], $
                                /remove_all ) )
                            print, file 
                        ...
                        if ( temp[0] NE '' ) then begin 
                            base_struc[i].ssp_loc = strcompress( temp[0], $
                                /remove_all ) 
                            base_struc[i].ssp_find = 1 
