...
pro hvdisp_plot_sig_index, result_list, index_name, suffix=suffix, $
    ...
    if keyword_set( red_include ) then begin 
        ...
        z0_include  = where( red_include EQ 0 ) 
        z1_include  = where( red_include EQ 1 ) 
        z2_include  = where( red_include EQ 2 ) 
        z3_include  = where( red_include EQ 3 ) 
    endif else begin 
        num_include = 3
        z0_include  = -1
        z1_include  = 1 
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    min_index = min( index_resarr.result.( index_num ) )  
    max_index = max( index_resarr.result.( index_num ) )  
    max_error = max( index_resarr.result.( error_num ) )  
